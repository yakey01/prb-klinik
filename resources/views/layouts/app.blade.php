...
<body x-data="{ mobileNav: false }">
    ...
    <nav style="background:rgba(17,36,28,.95); backdrop-filter:blur(16px); border-bottom:1px solid var(--line); position:sticky; top:0; z-index:50;">
        <div style="max-width:1400px; margin:0 auto; padding:0 1.5rem;">
            ...
            @php
                ...
                // Badge: pengajuan DISETUJUI menunggu direalisasikan jadi PO (siap belanja).
                $siapBelanjaCount = \Schema::hasTable('pengajuan_pengadaan')
                    ? \App\Models\PengajuanPengadaan::where('status', 'disetujui')->count() : 0;
            @endphp
            <div style="display:flex; gap:.15rem; margin-top:.25rem; align-items:stretch;" id="nav-tabs">
                ...
                <div class="nav-group" style="position:relative;">
                    <button onclick="navToggle('dd-pengadaan',this)" class="nav-tab {{ $inPengadaan ? 'active' : '' }}" style="background:none;border:none;cursor:pointer;" id="btn-pengadaan">
                        ...
                        Pengadaan
                        @if($siapBelanjaCount > 0)<span title="{{ $siapBelanjaCount }} pengajuan disetujui — siap dibelanjakan" style="display:inline-flex;align-items:center;justify-content:center;min-width:16px;height:16px;padding:0 4px;margin-left:.15rem;border-radius:999px;background:linear-gradient(180deg,#f2c668,#d9a441);color:#1a0e00;font-size:.6rem;font-weight:900;box-shadow:0 0 8px rgba(217,164,65,.6);">{{ $siapBelanjaCount }}</span>@endif
    ...
    @livewireScripts
    <script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.7/dist/chart.umd.min.js" data-navigate-once></script>
    @stack('scripts')
    {{-- data-navigate-once: blok ini hanya mendefinisikan fungsi global (navToggle,
         showToast) + mendaftar listener sekali (toast, offline, dropdown, hook 419).
         Tanpa ini, setiap wire:navigate me-run ulang → listener menumpuk → toast DOBEL
         + kebocoran listener. window persist lintas-navigate, jadi global tetap ada. --}}
