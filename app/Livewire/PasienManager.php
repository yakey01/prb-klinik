...
class PasienManager extends Component
{
    ...
    public function pasienList()
    {
        ...
        if ($this->filterDiagnosis) {
            $q->where('kategori_diagnosis', $this->filterDiagnosis);
        }
        if ($this->search) {
            $q->where(function ($q) {
    ...
    public function drawerPnl(): array
    {
        ...
        $row = DB::table('item_pengambilan as ip')
            ...
            ->selectRaw('
                ...
                COALESCE(SUM(ip.jumlah_unit * ip.harga_klaim_bpjs_snapshot * '.Obat::jfSql('ip.faktor_jasa_farmasi_snapshot').'), 0) as total_klaim,
    ...
    {
        if (! $this->drawerPasienId || ! $this->drawerPasien) {
            return collect();
        }
    ...
    public function stats(): array
    {
        ...
        return [
            ...
            'baru_bulan_ini' => Pasien::whereBetween('created_at', Periode::bulan(now()->year, now()->month))->count(),
        ];
    }
    ...
    public function updatedFilterDiagnosis(): void
    {
        $this->resetPage();
    }
    ...
    public function save(): void
    {
        $this->validate([
            ...
            // Unik HANYA di antara pasien HIDUP — No.BPJS milik pasien yang sudah
            // dihapus (soft delete) boleh dipakai lagi (akan dipulihkan, lihat save()).
            'no_bpjs' => ['required', 'string', 'digits:13', Rule::unique('pasien', 'no_bpjs')->whereNull('deleted_at')->ignore($this->editId)],
        ...
        if ($this->editId) {
            ...
            // No.BPJS ini pernah dipakai pasien yang DIHAPUS (soft delete) → PULIHKAN
            // pasien lama + perbarui datanya, bukan bikin baru. Ini menghindari bentrok
            // unique index DB (baris terhapus masih menempati No.BPJS) sekaligus
            // mengembalikan riwayat pasien (resep, pengambilan) yang lama.
            $trashed = Pasien::onlyTrashed()->where('no_bpjs', $this->no_bpjs)->first();
            if ($trashed) {
                $trashed->restore();
                $trashed->update($data);
                $pasienId = $trashed->id;
                ActivityLog::record('dibuat', "Pulihkan pasien: {$this->nama} (No.BPJS pernah dihapus)", 'pasien', $trashed->id);
            } else {
