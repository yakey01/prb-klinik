...
class LaporanBulanan extends Component
{
    ...
    public function ringkasan(): array
    {
        ...
        $proyeksiBpjs = (float) \DB::table('item_pengambilan as ip')
            ...
            ->where('po.status', 'selesai')
            ->where('o.tipe_obat', 'kronis')
            ->sum(\DB::raw('ip.jumlah_unit * ip.harga_klaim_bpjs_snapshot * '.Obat::jfSql('ip.faktor_jasa_farmasi_snapshot').''));
    ...
    public function detailBpjs()
    {
        ...
        $rows = \DB::table('item_pengambilan as ip')
            ...
            ->select(
                ...
                \DB::raw('SUM(ip.jumlah_unit * ip.harga_klaim_bpjs_snapshot * '.Obat::jfSql('ip.faktor_jasa_farmasi_snapshot').') AS proyeksi'),
    ...
    public function topLaba()
    {
        ...
        return \DB::table('item_pengambilan as ip')
            ...
            ->select(
                ...
                \DB::raw('SUM(ip.jumlah_unit * ip.harga_klaim_bpjs_snapshot * '.Obat::jfSql('ip.faktor_jasa_farmasi_snapshot').') AS proyeksi'),
                \DB::raw('SUM(ip.jumlah_unit * ip.harga_beli_snapshot) AS biaya'),
                \DB::raw('SUM(ip.jumlah_unit * ip.harga_klaim_bpjs_snapshot * '.Obat::jfSql('ip.faktor_jasa_farmasi_snapshot').')
    ...
    public function tren(): array
    {
        ...
        $kron = \DB::table('item_pengambilan as ip')
            ...
            ->where('po.status', 'selesai')
            ->where('o.tipe_obat', 'kronis')
            ->selectRaw("DATE_FORMAT(po.tanggal_pengambilan, '%Y-%m') AS ym, SUM(ip.jumlah_unit * ip.harga_klaim_bpjs_snapshot * ".Obat::jfSql('ip.faktor_jasa_farmasi_snapshot').') AS v')
            ->groupBy('ym')->pluck('v', 'ym');
    ...
    public function exportCsv(): StreamedResponse
    {
        ...
        return response()->streamDownload(function () use ($ringkasan, $detailBpjs, $detailNK, $periode) {
            ...
            fclose($handle);
        }, 'laporan_'.$this->tahun.'_'.str_pad((string) $this->bulan, 2, '0', STR_PAD_LEFT).'.csv');
    }
